...
{#if open}
  <div class="console-backdrop" role="presentation" onclick={(event) => event.currentTarget === event.target && closeConsole()}>
    <section class="console-dialog" role="dialog" aria-modal="true" aria-labelledby="server-console-title">
      ...
      {#if error}<div class="console-notice danger" role="alert">{error}</div>{/if}
...
<style>
  ...
  .console-notice{padding:9px 11px;border:1px solid var(--border-soft);border-radius:8px;font-size:10px}.console-notice.danger{border-color:#62343a;background:var(--danger-bg)}.console-notice.warning{border-color:#5f5125;background:var(--warning-bg)}.console-notice.success{border-color:var(--accent-border);background:var(--accent-soft)}
