<script lang="ts">
  ...
  function friendlyError(value: unknown) {
    return String(value).replace(/^Error:\s*/i, '').trim() || 'Client Setup failed. Try again.';
  }
  ...
  async function refresh() {
    ...
    catch (e) { status = null; error = friendlyError(e); }
    finally { busy = false; }
  }
...
<section class="client-page">
  ...
  {#if error}<div class="notice error" role="alert"><strong>Client Setup problem</strong><span>{error}</span></div>{/if}
