<script lang="ts">
  ...
  async function copyPath(value: string, key: string) {
    if (!value) return;
    if (await copyText(value)) {
      copiedPath = key;
      window.setTimeout(() => { if (copiedPath === key) copiedPath = ''; }, 1600);
    }
  }
...
<section class="client-page">
  ...
  {#if !loaded}
    <section class="card state-card" aria-live="polite"><strong>Checking Modrinth…</strong><span>Looking for the profile used with LazyBuilder.</span></section>
  {:else if !status}
    <section class="card state-card unavailable">
      <strong>Client information unavailable</strong>
      <span>LazyBuilder could not inspect Modrinth right now. Review the error above, then retry when the profile location is available.</span>
      <button class="secondary" disabled={busy} onclick={refresh}>{busy ? 'Checking…' : 'Try again'}</button>
    </section>
  {:else}
    <section class="card">
      <div class="card-head">
      ...
      {#if status.selectedProfile}
        <div class="path-panel">
          ...
          <div><span>Profile location</span><code>{status.selectedProfile.path}</code><button class="copy-path" onclick={() => copyPath(status!.selectedProfile!.path, 'profile')}>{copiedPath === 'profile' ? 'Copied' : 'Copy'}</button></div>
          <div><span>Mods location</span><code>{status.selectedProfile.modsPath}</code><button class="copy-path" onclick={() => copyPath(status!.selectedProfile!.modsPath, 'mods')}>{copiedPath === 'mods' ? 'Copied' : 'Copy'}</button></div>
          <div><span>Modrinth data folder</span><code>{status.selectedProfile.modrinthRoot}</code><button class="copy-path" onclick={() => copyPath(status!.selectedProfile!.modrinthRoot, 'root')}>{copiedPath === 'root' ? 'Copied' : 'Copy'}</button></div>
        </div>
        <div class="meta">
          ...
          <div class="component">
            <div>
              <div class="component-title">
                ...
                {#if item.id === 'performance-manager'}<span class="experimental-badge">Experimental</span>{/if}
              </div>
              <small title={item.targetFile}>{item.id === 'performance-manager' ? 'Experimental client performance component' : 'LazyBuilder client component'}</small>
            </div>
            <span class:ok={item.state === 'Installed'}>{item.state}</span>
          </div>
        {/each}
      </div>
      ...
      <footer>
        ...
        <button class="primary" disabled={busy || !status.selectedProfile?.compatible} title={!status.selectedProfile ? 'Select a Modrinth profile first.' : !status.selectedProfile.compatible ? 'Choose a compatible Minecraft/Fabric profile before syncing.' : undefined} onclick={syncClient}>{busy ? 'Syncing…' : 'Sync client'}</button>
      </footer>
    </section>
  {/if}
</section>
...
<style>
  ...
  .card{border:1px solid var(--border-soft);border-radius:10px;background:var(--surface);padding:14px}.state-card{min-height:120px;display:grid;place-content:center;justify-items:center;gap:5px;text-align:center;color:var(--muted)}.state-card strong{color:var(--text-soft);font-size:12px}.state-card span{max-width:520px;font-size:10px;line-height:1.45}.state-card .secondary{margin-top:7px}.state-card.unavailable{border-style:dashed}
  ...
  .path-panel{display:grid;gap:7px;margin-top:13px;padding:10px;border:1px solid var(--border-soft);border-radius:8px;background:var(--surface-2)}.path-panel>div{display:grid;grid-template-columns:120px minmax(0,1fr) auto;gap:10px;align-items:center}.path-panel span{color:var(--muted);font-size:9px}.path-panel strong,.path-panel code{font-size:9px;overflow-wrap:anywhere}.copy-path{min-height:26px;padding:4px 7px;border:1px solid var(--border-soft);border-radius:6px;background:var(--surface-3);color:var(--text-soft);font-size:8px;font-weight:650;cursor:pointer}.meta{display:flex;flex-wrap:wrap;gap:7px;margin-top:8px}.meta span{padding:3px 6px;border-radius:5px;background:var(--surface-2);color:var(--muted);font-size:9px}
  .components{margin-top:13px;border-top:1px solid var(--border-soft)}.component{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 0;border-bottom:1px solid var(--border-soft)}.component>div{display:grid;gap:2px}.component-title{display:flex;align-items:center;flex-wrap:wrap;gap:6px}.component strong{font-size:11px}.component small{font-size:9px;color:var(--muted)}.experimental-badge{padding:2px 5px;border-radius:999px;background:var(--warning-bg);color:#ebd9aa;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.03em}.component>span{font-size:10px;color:#ebd9aa}.component>span.ok{color:#a8e5b8}footer{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;padding-top:12px}footer p{margin:0;max-width:600px;color:var(--muted-2);font-size:9px;line-height:1.5}code{font-family:ui-monospace,SFMono-Regular,Consolas,monospace}
  @media(max-width:760px){.page-head,footer{align-items:stretch;flex-direction:column}.profile-controls{grid-template-columns:1fr}.path-panel>div{grid-template-columns:1fr}.copy-path{justify-self:start}}
</style>
