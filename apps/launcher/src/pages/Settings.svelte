<script lang="ts">
  ...
  async function loadClient() {
    clientError = '';
    try {
      clientStatus = await runtimeProduct.client.status();
      selectedClientPath = clientStatus.selectedProfile?.path ?? '';
    } catch (e) {
      clientStatus = null;
      clientError = friendlyError(e);
    }
  }
  ...
  async function selectClientProfile() {
    ...
    try {
      clientStatus = await runtimeProduct.client.selectProfile(selectedClientPath);
      selectedClientPath = clientStatus.selectedProfile?.path ?? selectedClientPath;
      clientMessage = 'Modrinth profile selected. Sync Client will install only LazyBuilder-owned mods.';
  ...
  async function syncClient() {
    ...
    clientBusy = true; clientError = ''; clientMessage = '';
    try {
      clientStatus = await runtimeProduct.client.sync();
      selectedClientPath = clientStatus.selectedProfile?.path ?? selectedClientPath;
...
<section class="settings-page">
  ...
  <section class="settings-group client-group">
    ...
    <div class="group-content">
      ...
      {:else if !clientStatus.modrinthDetected}
        <div class="client-card warning-card"><strong>Modrinth App not detected</strong><p>Install or open Modrinth App once, then return here and refresh Settings.</p></div>
      {:else}
        <div class="client-card">
          ...
          <label class="profile-picker">
            ...
            <select bind:value={selectedClientPath} disabled={clientBusy} onchange={selectClientProfile}>
              <option value="">Choose profile…</option>
              {#each clientStatus.profiles as item}
                <option value={item.path} disabled={!item.compatible}>{item.name} — {item.gameVersion ?? 'Unknown'} / {item.loader ?? 'Unknown'}{item.compatible ? '' : ' (incompatible)'}</option>
              {/each}
            </select>
          </label>

          {#if clientStatus.selectedProfile}
            <div class="profile-meta"><span>Minecraft {clientStatus.selectedProfile.gameVersion ?? 'Unknown'}</span><span>{clientStatus.selectedProfile.loader ?? 'Unknown loader'}</span></div>
          {/if}
          ...
          <div class="client-footer">
            <p>Other Modrinth mods are never modified. Sync only replaces files beginning with LazyBuilder's own managed mod names.</p>
            <button class="primary" disabled={clientBusy || !clientStatus.selectedProfile?.compatible} onclick={syncClient}>{clientBusy ? 'Syncing…' : clientStatus.ready ? 'Sync Client' : 'Sync Client'}</button>
          </div>
        </div>
      {/if}
    </div>
...
<style>
  ...
  @media(max-width:760px){.settings-group{grid-template-columns:1fr;gap:10px}.settings-footer,.client-footer{align-items:stretch;flex-direction:column}.settings-footer>div{text-align:left}.memory-head{align-items:flex-start;flex-direction:column}}
</style>
