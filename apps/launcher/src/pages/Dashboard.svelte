<script lang="ts">
  ...
  function category(message: string) {
    ...
    if (value.includes('workspace') || value.includes('directory') || value.includes('path')) return 'Server files';
  ...
  async function action(run: () => Promise<void>) {
    if (busy) return; busy = true; notice = ''; error = null;
  ...
  async function stopDetachedProcess() {
    ...
    try {
      ...
      confirmExternalStop = false;
      notice = result.message;
      await refreshAll();
    }
    catch (value) { error = presentRuntimeError(value, 'Could not recover the external server process.'); }
    finally { busy = false; }
  }
  async function loadLog() {
    if (logBusy) return; logBusy = true;
    try { logTail = await runtimeProduct.server.logTail(snapshot.logPath || ''); logOpen = true; }
    catch (value) { error = presentRuntimeError(value, 'Could not load the server log.'); } finally { logBusy = false; }
  }
  onMount(() => {
    let disposed = false;
...
{#if confirmExternalStop}<div class="modal-backdrop" role="presentation" onclick={(event) => event.currentTarget === event.target && !busy && (confirmExternalStop = false)}><div use:dialogFocus={{ onEscape: () => (confirmExternalStop = false), initialFocusSelector: '.secondary', escapeDisabled: busy }} class="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="stop-external-title"><header><div><h2 id="stop-external-title">Stop external server?</h2><p>LazyBuilder will stop the verified Paper process that is still running from an earlier launcher session.</p></div><button class="icon-button" aria-label="Close stop confirmation" disabled={busy} onclick={() => (confirmExternalStop = false)}>×</button></header><div class="confirm-copy"><strong>Server files are not modified.</strong><span>After the process stops, start the server again from LazyBuilder to restore normal console and lifecycle control.</span></div><footer><button class="secondary" disabled={busy} onclick={() => (confirmExternalStop = false)}>Cancel</button><button class="danger" disabled={busy} onclick={stopDetachedProcess}>{busy ? 'Stopping…' : 'Stop external server'}</button></footer></div></div>{/if}

{#if logOpen}<div class="modal-backdrop" role="presentation" onclick={(event) => event.currentTarget === event.target && (logOpen = false)}><div use:dialogFocus={{ onEscape: () => (logOpen = false), initialFocusSelector: '.primary', escapeDisabled: logBusy }} class="log-dialog" role="dialog" aria-modal="true" aria-labelledby="server-log-title"><header><div><h2 id="server-log-title">Server log</h2><p>{logTail.path ? logTail.path.split(/[\\/]/).pop() : 'latest.log'}{logTail.truncated ? ' · showing recent lines' : ''}</p></div><button class="icon-button" aria-label="Close server log" onclick={() => (logOpen = false)}>×</button></header><pre>{logTail.content || 'No server log output is available yet.'}</pre><footer><button class="secondary" disabled={logBusy} onclick={loadLog}>{logBusy ? 'Refreshing…' : 'Refresh'}</button><button class="primary" onclick={() => (logOpen = false)}>Done</button></footer></div></div>{/if}
