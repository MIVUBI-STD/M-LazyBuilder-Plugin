<script lang="ts">
  ...
  import { controlledMenu } from '../app/controlledMenu';
  ...
  let menuServerId = $state<string | null>(null);
  let libraryConsoleWorkspaceId = $state<string | null>(null);
  let libraryConsoleServerName = $state('Server');
  let libraryRuntimeBusyId = $state<string | null>(null);
  let stopCandidate = $state<WorkspaceEntry | null>(null);
  let stopCandidateDetached = $state(false);
  ...
  let adoptionPlan = $state<AdoptionPlan | null>(null);
  let managementServer = $state<WorkspaceEntry | null>(null);
  let managementMode = $state<ManagementMode>(null);
  let managementBusy = $state(false);
  let managementError = $state('');
  let locateCandidate = $state('');
  ...
  function shouldLocateWorkspace(value: unknown) {
    return value instanceof RuntimeError && (value.action === 'LOCATE_WORKSPACE' || value.code === 'WORKSPACE_UNAVAILABLE');
  }

  function clearSurfaceFeedback() {
    surfaceError = '';
    surfaceNotice = '';
  }
  ...
  async function openServer(server: WorkspaceEntry) {
    ...
    try { await onOpenServer(server); }
      ...
      if (shouldLocateWorkspace(value)) beginLocate(server, message);
  ...
  async function reconnectServerLocation() {
    ...
    try {
      ...
      await onChanged();
      clearSurfaceFeedback();
      surfaceNotice = `${relocated.name} was reconnected successfully.`;
    } catch (value) { managementError = friendlyError(value); }
...
<main class="library-content">
  ...
  {#if surfaceError}<div class="error-box" role="alert">{surfaceError}</div>{/if}
  {#if surfaceNotice}<div class="notice-box success" aria-live="polite">{surfaceNotice}</div>{/if}
  {#if recent.length > 0}
      ...
      <div class="server-grid">
        {#each visibleServers() as server}
          ...
          <div class="server-tile">
            ...
            <div use:controlledMenu={{ open: menuServerId === server.id, onOpen: () => (menuServerId = server.id), onClose: () => { if (menuServerId === server.id) menuServerId = null; } }} class="server-menu-wrap"><button class="server-menu-button" aria-label={`Manage ${server.name}`} aria-expanded={menuServerId === server.id} onclick={() => (menuServerId = menuServerId === server.id ? null : server.id)}>•••</button>
              {#if menuServerId === server.id}<div class="server-menu" role="menu"><button role="menuitem" onclick={() => void openServer(server)}>Open</button>{#if canOpenRuntimeConsole(runtime)}<button role="menuitem" onclick={() => openLibraryConsole(server, runtime)}>Open console</button><button role="menuitem" disabled={libraryRuntimeBusyId === server.id} onclick={() => requestStopRuntime(server, runtime)}>{libraryRuntimeBusyId === server.id ? 'Stopping…' : 'Stop server'}</button><div class="menu-divider"></div>{:else if runtime?.state === 'Detached'}<button role="menuitem" class="danger-menu-item" disabled={libraryRuntimeBusyId === server.id} onclick={() => requestStopRuntime(server, runtime)}>{libraryRuntimeBusyId === server.id ? 'Stopping…' : 'Stop external server'}</button><div class="menu-divider"></div>{/if}<button role="menuitem" onclick={() => void openServerFolder(server)}>Open folder</button><button role="menuitem" disabled={!canMutateLibraryServer(runtime)} onclick={() => beginLocate(server)}>Locate moved server…</button><div class="menu-divider"></div><button role="menuitem" disabled={!canMutateLibraryServer(runtime)} onclick={() => beginDuplicate(server)}>Duplicate server</button><div class="menu-divider"></div><button role="menuitem" disabled={!canMutateLibraryServer(runtime)} onclick={() => beginRemove(server)}>Remove from library</button><button role="menuitem" class="danger-menu-item" disabled={!canMutateLibraryServer(runtime)} onclick={() => beginDelete(server)}>Delete server…</button></div>{/if}
            </div>
          </div>
        {/each}
      </div>
...
<style>
  ...
  .primary-button,.secondary-button,.ghost-button,.icon-button,.danger-button{min-height:36px;border-radius:8px;padding:8px 13px;font-weight:650;cursor:pointer}.primary-button{border:1px solid var(--accent);background:var(--accent);color:var(--accent-ink)}.secondary-button{border:1px solid var(--border);background:var(--surface-2);color:var(--text)}.ghost-button,.icon-button{border:0;background:transparent;color:var(--text-soft)}.icon-button{font-size:20px}.danger-button{border:1px solid #8e3840;background:#7a2d34;color:#fff}button:disabled{opacity:.5;cursor:default}.error-box,.notice-box{margin-bottom:12px;padding:11px 13px;border-radius:8px;font-size:12px}.error-box{border:1px solid #70343a;background:var(--danger-bg);color:#ffd9dc}.notice-box.success{border:1px solid var(--accent-border);background:var(--accent-soft);color:#b9e5c7}
