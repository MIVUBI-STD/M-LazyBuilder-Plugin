...
function serverSnapshot(): ServerSnapshot {
  const offline = previewPage() === 'Plugins' || previewPage() === 'Settings';
  return {
    state: offline ? 'Offline' : 'Online',
    health: offline ? 'Offline' : 'Good',
    cpuLoadPercent: offline ? 0 : 17,
    usedMemoryBytes: offline ? 0 : 2.4 * 1024 ** 3,
    maxMemoryBytes: 6 * 1024 ** 3,
    pid: offline ? null : 14872,
    logPath: 'D:\\LazyBuilder\\MIVUBI Build Server\\logs\\latest.log'
  };
}

const diagnostics: DiagnosticSummary = {
  launcherVersion: '0.1.0-preview',
  launcherLogPath: 'C:\\Users\\Builder\\AppData\\Local\\LazyBuilder\\logs\\launcher.log',
  workspaceName: activeWorkspace.name,
...
const clientStatus: ClientIntegrationStatus = {
  ...
  ready: true,
  message: 'LazyBuilder client components are synchronized with this profile.'
};
...
export const runtimePreviewProduct = {
  ...
  workspace: {
    ...
    provisioningStatus: async () => provisioningStatus(),
    ...
    adopt: async (_rootPath: string, name?: string | null) => ({ ...activeWorkspace, name: name || 'Existing Server' }),
    activate: async (id: string) => recentWorkspaces.find((item) => item.id === id) ?? activeWorkspace,
    close: async () => undefined,
    openFolder: async (_id: string) => undefined,
    ...
    removeFromLibrary: async (_id: string) => undefined,
    delete: async (_id: string, _typedDisplayName: string) => undefined
  },
  server: {
    preflight: async (): Promise<ServerPreflight> => ({ ready: true, workspace: activeWorkspace.path, serverDirectory: `${activeWorkspace.path}\\server`, paperJar: `${activeWorkspace.path}\\server\\paper.jar`, worldsDirectory: `${activeWorkspace.path}\\server\\worlds`, javaPath: 'C:\\LazyBuilder\\runtime\\java.exe', javaVersion: '21.0.8', logDirectory: `${activeWorkspace.path}\\logs`, issues: [] }),
    snapshot: async () => serverSnapshot(),
    ...
    resources: async () => resourceProfile,
    saveResources: async (request: { maxMemoryMb: number }) => ({ ...resourceProfile, currentMaxMemoryMb: request.maxMemoryMb })
  },
  plugins: {
    list: async () => pluginList,
    pickJar: async () => 'D:\\Downloads\\Plugin.jar',
    install: async (_jarPath: string) => pluginResult,
    update: async (pluginId: string, _jarPath: string) => ({ ...pluginResult, pluginId }),
    setEnabled: async (_pluginId: string, _enabled: boolean) => undefined,
    remove: async (_pluginId: string) => undefined,
    removeProblem: async (_pluginId: string, _jarFileName: string) => undefined,
    resolveDuplicates: async (pluginId: string, _keepJarFileName: string) => ({ ...pluginResult, pluginId })
  },
  client: {
    status: async () => clientStatus,
    selectProfile: async (_profilePath: string) => clientStatus,
    pickProfile: async () => clientStatus,
    sync: async () => clientStatus
  },
  worlds: {
    list: async () => worlds,
    ...
    tasks: async () => [completedTask()],
    task: async (_taskId: string) => completedTask(),
    archive: async (worldId: string) => completedTask(worldId),
    restore: async (worldId: string) => completedTask(worldId),
    backup: async (worldId: string) => completedTask(worldId),
    duplicate: async (request: { worldId: string }) => completedTask(request.worldId),
    export: async (request: { worldId: string }) => completedTask(request.worldId),
    delete: async (request: { worldId: string }) => completedTask(request.worldId),
    pickImport: async () => 'D:\\Imports\\Example World.zip',
    uploadImport: async (_filePath: string) => 'preview-import.zip',
    import: async (_request: unknown) => completedTask('preview-import')
  }
};
