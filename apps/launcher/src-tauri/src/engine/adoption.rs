...
fn load_pending_adoptions() -> Result<Vec<PendingAdoption>, String> {
    ...
    if !persistence::metadata_entry_exists(&path, "pending server adoption intent")? {
        return Ok(Vec::new());
    }
    let entries = persistence::read_json(&path, "pending server adoption intent")
        .map_err(|error| format!("Could not parse pending server adoptions: {error}"))?;
...
fn save_pending_adoptions(entries: &[PendingAdoption]) -> Result<(), String> {
    ...
    if entries.is_empty() {
        persistence::safe_path::remove_regular_file_if_present(
            &path,
            "pending server adoption intent",
        )?;
        cleanup_pending_adoption_recovery_files(&path)?;
        return Ok(());
    }
...
fn recover_pending_adoption_file(path: &Path) -> Result<(), String> {
    persistence::recover_atomic_file(path, "pending server adoption intent")
}

fn cleanup_pending_adoption_recovery_files(path: &Path) -> Result<(), String> {
    persistence::cleanup_recovery_files(path, "pending server adoption intent")
}
