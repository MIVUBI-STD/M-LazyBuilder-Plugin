...
fn load_pending_adoptions() -> Result<Vec<PendingAdoption>, String> {
    ...
    if !metadata_entry_exists(&path, "pending server adoption intent")? { return Ok(Vec::new()); }
    ensure_regular_metadata_file(&path, "pending server adoption intent")?;
    let text = fs::read_to_string(&path).map_err(|error| format!("Could not read pending server adoptions: {error}"))?;
    let entries = serde_json::from_str(&text).map_err(|error| format!("Could not parse pending server adoptions: {error}"))?;
...
fn save_pending_adoptions(entries: &[PendingAdoption]) -> Result<(), String> {
    let path = pending_adoptions_path()?;
    if entries.is_empty() {
        remove_metadata_file_if_exists(&path, "pending server adoption intent")?;
        cleanup_pending_adoption_recovery_files(&path)?;
        return Ok(());
    }
    if let Some(parent) = path.parent() { fs::create_dir_all(parent).map_err(|error| error.to_string())?; }
    let incoming = path.with_extension("json.incoming");
    remove_metadata_file_if_exists(&incoming, "pending server adoption staging metadata")?;
    let text = serde_json::to_string_pretty(entries).map_err(|error| error.to_string())?;
    {
        let mut file = OpenOptions::new().create_new(true).write(true).open(&incoming)
            .map_err(|error| format!("Could not write pending server adoption intent: {error}"))?;
    ...
    replace_pending_adoption_file(&incoming, &path)
}

fn recover_pending_adoption_file(path: &Path) -> Result<(), String> {
    let previous = path.with_extension("json.previous");
    let incoming = path.with_extension("json.incoming");
    if metadata_entry_exists(path, "pending server adoption intent")? {
        ensure_regular_metadata_file(path, "pending server adoption intent")?;
        return Ok(());
    }
    if metadata_entry_exists(&previous, "previous pending server adoption intent")? {
        ensure_regular_metadata_file(&previous, "previous pending server adoption intent")?;
        fs::rename(&previous, path).map_err(|error| format!("Could not restore previous pending server adoption intent: {error}"))?;
        return Ok(());
    }
    if metadata_entry_exists(&incoming, "pending server adoption staging metadata")? {
        ensure_regular_metadata_file(&incoming, "pending server adoption staging metadata")?;
        fs::rename(&incoming, path).map_err(|error| format!("Could not publish recovered pending server adoption intent: {error}"))?;
    }
    Ok(())
}

fn cleanup_pending_adoption_recovery_files(path: &Path) -> Result<(), String> {
    remove_metadata_file_if_exists(&path.with_extension("json.previous"), "previous pending server adoption intent")?;
    remove_metadata_file_if_exists(&path.with_extension("json.incoming"), "pending server adoption staging metadata")
}

fn replace_pending_adoption_file(source: &Path, destination: &Path) -> Result<(), String> {
    ensure_regular_metadata_file(source, "pending server adoption staging metadata")?;
    if metadata_entry_exists(destination, "pending server adoption intent")? {
        ensure_regular_metadata_file(destination, "pending server adoption intent")?;
        let previous = destination.with_extension("json.previous");
        remove_metadata_file_if_exists(&previous, "previous pending server adoption intent")?;
        fs::rename(destination, &previous).map_err(|error| format!("Could not preserve previous pending server adoption intent: {error}"))?;
        match fs::rename(source, destination) {
            ...
            Err(publish_error) => match fs::rename(&previous, destination) {
                Ok(()) => Err(format!("Could not publish pending server adoption intent; previous intent was restored: {publish_error}")),
                Err(rollback_error) => Err(format!("Could not publish pending server adoption intent ({publish_error}) and could not restore previous intent ({rollback_error}). Recovery files were preserved.")),
            },
        }
    } else {
        fs::rename(source, destination).map_err(|error| format!("Could not publish pending server adoption intent: {error}"))
    }
}
...
mod tests {
    ...
    fn temp_metadata_path(label: &str) -> PathBuf {
        ...
        root.join("pending-adoptions.json")
    }
    ...
    #[test]
    fn adoption_intent_recovery_prefers_previous_committed_copy() {
        let path = temp_metadata_path("adoption-previous");
        let previous = path.with_extension("json.previous");
        let incoming = path.with_extension("json.incoming");
        fs::write(&previous, b"[]").unwrap();
        fs::write(&incoming, b"[]").unwrap();
        recover_pending_adoption_file(&path).unwrap();
        assert!(path.exists());
        assert!(incoming.exists());
        cleanup_pending_adoption_recovery_files(&path).unwrap();
        assert!(!incoming.exists());
        let _ = fs::remove_dir_all(path.parent().unwrap());
    }

    #[test]
    fn adoption_intent_incoming_recovers_without_committed_copy() {
        let path = temp_metadata_path("adoption-incoming");
        let incoming = path.with_extension("json.incoming");
        fs::write(&incoming, b"[]").unwrap();
        recover_pending_adoption_file(&path).unwrap();
    ...
    #[test]
    fn malformed_adoption_intent_preserves_recovery_evidence() {
        let path = temp_metadata_path("adoption-malformed");
        let previous = path.with_extension("json.previous");
        let incoming = path.with_extension("json.incoming");
        ...
        fs::write(&incoming, b"[]").unwrap();
        recover_pending_adoption_file(&path).unwrap();
        assert!(serde_json::from_str::<Vec<PendingAdoption>>(&fs::read_to_string(&path).unwrap()).is_err());
