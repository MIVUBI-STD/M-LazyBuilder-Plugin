...
impl Default for ServerManagerState {
    fn default() -> Self { Self { child: Mutex::new(None), system: Mutex::new(System::new_all()), runtime_state: Arc::new(Mutex::new("Offline".into())), expected_stop: Arc::new(AtomicBool::new(false)), startup_started_at: Mutex::new(None), active_log_path: Arc::new(Mutex::new(String::new())) } }
}
...
impl ServerManagerState {
    pub fn preflight(&self) -> ServerPreflight {
        ...
        let workspace = match paths::workspace_root() { Ok(value) => { workspace_display = value.display().to_string(); Some(value) }, Err(error) => { issues.push(error); None } };
        ...
        match paths::worlds_dir() { Ok(value) => worlds_display = value.display().to_string(), Err(error) => issues.push(error) }
        match self.detached_process() { Ok(Some(pid)) => issues.push(format!("A previous managed Paper process is still running as PID {pid}. Close it before starting another instance.")), Ok(None) => {}, Err(error) => issues.push(format!("Process recovery check: {error}")) }
    ...
    pub fn snapshot(&self) -> Result<ServerSnapshot, String> {
        let options = load_options()?; let runtime_resources = resource_settings::runtime_resources()?; let mut child_guard = self.child.lock().map_err(|_| "server state lock poisoned".to_string())?;
        let Some(child) = child_guard.as_mut() else {
            if let Some(pid) = self.detached_process()? { let (cpu, memory) = self.process_usage(pid)?; return Ok(ServerSnapshot { state: "Detached".into(), health: "Warning".into(), cpu_load_percent: cpu, used_memory_bytes: memory, max_memory_bytes: runtime_resources.max_memory_mb * 1024 * 1024, pid: Some(pid), log_path: self.current_log_path() }); }
            let state = self.runtime_state.lock().map_err(|_| "server runtime state lock poisoned".to_string())?.clone(); return Ok(offline_like_snapshot(&options, state, self.current_log_path()));
        };
        if child.try_wait().map_err(|error| error.to_string())?.is_some() {
            let pid = child.id(); *child_guard = None; let _ = remove_process_marker_if_matches(pid); let state = if self.expected_stop.load(Ordering::SeqCst) { "Offline" } else { "Crashed" }; set_runtime_state(&self.runtime_state, state)?; *self.startup_started_at.lock().map_err(|_| "startup state lock poisoned".to_string())? = None; return Ok(offline_like_snapshot(&options, state.into(), self.current_log_path()));
        }
        let pid = child.id(); let state = self.runtime_state.lock().map_err(|_| "server runtime state lock poisoned".to_string())?.clone();
        if state == "Crashed" && self.detached_process()?.is_some() { let (cpu, memory) = self.process_usage(pid)?; return Ok(ServerSnapshot { state: "Detached".into(), health: "Warning".into(), cpu_load_percent: cpu, used_memory_bytes: memory, max_memory_bytes: runtime_resources.max_memory_mb * 1024 * 1024, pid: Some(pid), log_path: self.current_log_path() }); }
        let (cpu, memory) = self.process_usage(pid)?; let max_memory = runtime_resources.max_memory_mb * 1024 * 1024; let memory_ratio = if max_memory == 0 { 0.0 } else { memory as f64 / max_memory as f64 };
        let startup_timed_out = if state == "Starting" { self.startup_started_at.lock().map_err(|_| "startup state lock poisoned".to_string())?.as_ref().map(|started| started.elapsed() >= Duration::from_secs(options.startup_timeout_seconds)).unwrap_or(false) } else { false };
    ...
    pub fn start(&self) -> Result<(), String> {
        ...
        if let Some(child) = guard.as_mut() {
            ...
            let pid = child.id(); *guard = None; let _ = remove_process_marker_if_matches(pid);
        }
        if let Some(pid) = self.detached_process()? { return Err(format!("Refusing to start a second Paper instance. Previous managed process PID {pid} is still running.")); }
        let workspace = paths::workspace_root()?; paths::ensure_runtime_layout()?; let worlds_dir = paths::worlds_dir()?; let options = load_options()?; let resources = resource_settings::runtime_resources()?; let (server_dir, paper) = resolve_server_paths(&workspace, &options)?;
        ...
        *self.active_log_path.lock().map_err(|_| "log path lock poisoned".to_string())? = startup_log.display().to_string();
        ...
        let mut child = command.args(["-jar", &options.paper_jar]).arg("--universe").arg(&worlds_dir).arg("nogui")
    ...
    pub fn stop(&self) -> Result<(), String> {
        let options = load_options()?; let mut guard = self.child.lock().map_err(|_| "server state lock poisoned".to_string())?;
        ...
        let pid = child.id(); let deadline = Instant::now() + Duration::from_secs(options.graceful_stop_timeout_seconds);
        while Instant::now() < deadline { if child.try_wait().map_err(|error| error.to_string())?.is_some() { *guard = None; let _ = remove_process_marker_if_matches(pid); set_runtime_state(&self.runtime_state, "Offline")?; *self.startup_started_at.lock().map_err(|_| "startup state lock poisoned".to_string())? = None; return Ok(()); } thread::sleep(Duration::from_millis(100)); }
        child.kill().map_err(|error| error.to_string())?; let _ = child.wait(); *guard = None; let _ = remove_process_marker_if_matches(pid); set_runtime_state(&self.runtime_state, "Offline")?; *self.startup_started_at.lock().map_err(|_| "startup state lock poisoned".to_string())? = None; Ok(())
    }

    pub fn recover_detached(&self) -> Result<DetachedRecoveryResult, String> {
        let Some(pid_value) = self.detached_process()? else { return Err("No detached LazyBuilder Paper process marker exists.".into()); };
        ...
        let Some(process) = system.process(pid) else { drop(system); remove_process_marker_if_matches(pid_value)?; clear_legacy_process_identity(); return Ok(DetachedRecoveryResult { pid: pid_value, stopped: true, message: "The previous Paper process is no longer running. Its stale recovery marker was cleared.".into() }); };
        if !looks_like_managed_paper(process)? { return Err(format!("PID {pid_value} no longer matches the LazyBuilder-managed Paper command line. Recovery was blocked to avoid terminating an unrelated process.")); }
        self.expected_stop.store(true, Ordering::SeqCst); set_runtime_state(&self.runtime_state, "Stopping")?;
        ...
        while Instant::now() < deadline { thread::sleep(Duration::from_millis(100)); system.refresh_process(pid); if system.process(pid).is_none() { drop(system); remove_process_marker_if_matches(pid_value)?; clear_legacy_process_identity(); set_runtime_state(&self.runtime_state, "Offline")?; *self.startup_started_at.lock().map_err(|_| "startup state lock poisoned".to_string())? = None; return Ok(DetachedRecoveryResult { pid: pid_value, stopped: true, message: format!("Detached Paper PID {pid_value} was terminated and the controller is ready to start a new managed instance.") }); } }
    ...
    fn detached_process(&self) -> Result<Option<u32>, String> {
        let Some(mut marker) = read_process_marker()? else { return Ok(None); }; let pid = Pid::from_u32(marker.pid); let mut system = self.system.lock().map_err(|_| "system monitor lock poisoned".to_string())?; system.refresh_process(pid);
        let Some(process) = system.process(pid) else { drop(system); let _ = remove_process_marker_if_matches(marker.pid); clear_legacy_process_identity(); return Ok(None); };
        if marker.process_start_time != 0 { if process.start_time() != marker.process_start_time { drop(system); let _ = remove_process_marker_if_matches(marker.pid); clear_legacy_process_identity(); return Ok(None); } return Ok(Some(marker.pid)); }
        if !looks_like_managed_paper(process)? { drop(system); let _ = remove_process_marker_if_matches(marker.pid); clear_legacy_process_identity(); return Ok(None); }
        marker.process_start_time = process.start_time(); drop(system); write_process_marker_value(&marker)?; clear_legacy_process_identity(); Ok(Some(marker.pid))
    }
...
fn offline_like_snapshot(options: &ServerManagerOptions, state: String, log_path: String) -> ServerSnapshot { let max_memory_mb = resource_settings::runtime_resources().map(|value| value.max_memory_mb).unwrap_or(options.max_memory_mb); let health = if state == "Crashed" { "Critical" } else { "Offline" }; ServerSnapshot { state, health: health.into(), cpu_load_percent: 0.0, used_memory_bytes: 0, max_memory_bytes: max_memory_mb * 1024 * 1024, pid: None, log_path } }
...
fn process_marker_path() -> Result<PathBuf, String> { Ok(paths::lazybuilder_cache_dir()?.join("server-process.json")) }
...
fn write_process_marker(pid: u32) -> Result<(), String> { let pid_value = Pid::from_u32(pid); let mut system = System::new_all(); system.refresh_process(pid_value); let process = system.process(pid_value).ok_or_else(|| format!("Paper PID {pid} exited before its process marker could be recorded"))?; let marker = ServerProcessMarker { pid, process_start_time: process.start_time() }; write_process_marker_value(&marker)?; clear_legacy_process_identity(); Ok(()) }
fn write_process_marker_value(marker: &ServerProcessMarker) -> Result<(), String> { let path = process_marker_path()?; if let Some(parent) = path.parent() { fs::create_dir_all(parent).map_err(|error| error.to_string())?; } let text = serde_json::to_string_pretty(marker).map_err(|error| error.to_string())?; let temporary = path.with_extension("json.tmp"); fs::write(&temporary, text).map_err(|error| error.to_string())?; replace_file(&temporary, &path) }
fn read_process_marker() -> Result<Option<ServerProcessMarker>, String> { let path = process_marker_path()?; if !path.is_file() { clear_legacy_process_identity(); return Ok(None); } let text = fs::read_to_string(&path).map_err(|error| error.to_string())?; match serde_json::from_str(&text) { Ok(marker) => Ok(Some(marker)), Err(_) => { let _ = fs::remove_file(path); clear_legacy_process_identity(); Ok(None) } } }
fn remove_process_marker_if_matches(pid: u32) -> Result<(), String> { let path = process_marker_path()?; let Some(marker) = read_process_marker()? else { return Ok(()); }; if marker.pid == pid { fs::remove_file(path).map_err(|error| error.to_string())?; clear_legacy_process_identity(); } Ok(()) }
...
fn looks_like_managed_paper(process: &Process) -> Result<bool, String> { let process_name = process.name().to_ascii_lowercase(); let command_lower = process.cmd().join(" ").to_ascii_lowercase(); let worlds = paths::worlds_dir()?.display().to_string().to_ascii_lowercase(); Ok(process_name.contains("java") && command_lower.contains("-jar") && command_lower.contains("--universe") && command_lower.contains("nogui") && command_lower.contains(&worlds)) }
fn clear_legacy_process_identity() { if let Ok(root) = paths::lazybuilder_cache_dir() { let path = root.join("server-process-identity.json"); let _ = fs::remove_file(&path); let _ = fs::remove_file(path.with_extension("json.previous")); let _ = fs::remove_file(path.with_extension("json.tmp")); } }
