use crate::engine::{diagnostics, persistence};
...
fn load_journal(path: &Path) -> Result<VecDeque<OperationSnapshot>, String> {
    ...
    if !persistence::metadata_entry_exists(path, "Launcher operation journal")? {
...
fn recover_journal_file(path: &Path) -> Result<(), String> {
    persistence::recover_atomic_file(path, "Launcher operation journal")
}

fn cleanup_journal_recovery_files(path: &Path) -> Result<(), String> {
    persistence::cleanup_recovery_files(path, "Launcher operation journal")
}

fn read_journal_file(path: &Path) -> Result<VecDeque<OperationSnapshot>, String> {
    let journal: OperationJournal = persistence::read_json(
        path,
        "Launcher operation journal",
    )?;
    if journal.schema_version != OPERATION_JOURNAL_SCHEMA_VERSION {
        return Err(format!(
            "Launcher operation journal schema {} is unsupported by this build",
