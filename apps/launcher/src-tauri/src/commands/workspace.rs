...
pub async fn workspace_provision(app: AppHandle) -> CommandResult<provisioning::ProvisionResult> {
    ...
    let task = tauri::async_runtime::spawn_blocking(move || {
        ...
        let result = provisioning::provision_active_tracked(resource_dir.as_deref(), |phase, status, details| {
    ...
    match task.await {
        ...
        Err(error) => {
            let _ = app.state::<OperationRegistry>().require_recovery(&join_operation_id, OperationError {
                code: "TASK_FAILED".into(), message: "Server preparation task ended unexpectedly".into(), details: error.to_string(), recoverable: true,
            });
...
pub async fn workspace_update_paper(app: AppHandle) -> CommandResult<runtime_updates::RuntimeUpdateStatus> {
    ...
    let task = tauri::async_runtime::spawn_blocking(move || {
        ...
        match result {
            ...
            Err(message) => {
                ...
                if rollback_failed { let _ = operations.require_recovery(&operation_id, operation_error); }
                else { let _ = operations.fail(&operation_id, operation_error); }
                Err(error)
            }
        }
    });
    ...
    match task.await {
        ...
        Err(error) => {
            let _ = app.state::<OperationRegistry>().require_recovery(&join_operation_id, OperationError {
                code: "TASK_FAILED".into(), message: "Paper update task ended unexpectedly".into(), details: error.to_string(), recoverable: true,
            });
...
pub async fn workspace_adopt(app: AppHandle, root_path: String, name: Option<String>) -> CommandResult<WorkspaceEntry> {
    ...
    }).await.map_err(|error| CommandError::new("TASK_FAILED", format!("Server adoption task failed: {error}")))?
}
...
pub fn workspace_open_folder(id: String) -> CommandResult<()> {
    ...
    if !path.is_dir() {
        return Err(CommandError::new("WORKSPACE_UNAVAILABLE", format!("Server location is currently unavailable: {}", path.display())));
    }
...
#[tauri::command]
pub fn workspace_duplicate_estimate(state: State<'_, ServerManagerState>, id: String, parent_path: String) -> CommandResult<WorkspaceDuplicateEstimate> {
...
pub async fn workspace_duplicate(app: AppHandle, id: String, parent_path: String, name: String) -> CommandResult<WorkspaceEntry> {
    ...
    let operation = app.state::<OperationRegistry>().begin_exclusive("duplicate-server", &resource, false)
        .map_err(|error| CommandError::new("OPERATION_BUSY", error))?;
    let operation_id = operation.id.clone();
    let join_operation_id = operation.id.clone();
    let task_app = app.clone();
    ...
    let task = tauri::async_runtime::spawn_blocking(move || {
        ...
        operations.set_phase(
            &operation_id,
            "copying",
            "Copying server files",
            "The duplicate is staged and published only after the copy and workspace identity are validated.",
            Some(OperationProgress { current: 0, total: Some(estimate.source_bytes), unit: "bytes".into() }),
        ).map_err(CommandError::from)?;
        ...
        match workspace_registry::duplicate(&id, Path::new(&parent_path), &name) {
            ...
            Err(message) => {
                let error = CommandError::recoverable("DUPLICATE_FAILED", message, "Retry duplicate");
                fail_operation(&operations, &operation_id, &error, true);
                Err(error)
            }
        }
    ...
    match task.await {
        ...
        Err(error) => {
            let _ = app.state::<OperationRegistry>().require_recovery(&join_operation_id, OperationError {
                code: "TASK_FAILED".into(), message: "Server duplication task ended unexpectedly".into(), details: error.to_string(), recoverable: true,
            });
