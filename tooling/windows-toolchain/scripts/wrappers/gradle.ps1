...
$GradleEnvironmentScript = Join-Path $PSScriptRoot 'gradle-environment.ps1'
...
$CacheRoot = Join-Path $GradleEnvironment.Root "build-tools\gradle\$Version"
...
function Ensure-Gradle {
    ...
    try {
        ...
        $Actual = Get-FileDigest $Archive 'SHA256'
