...
public final class MaterialMutationPlanner {
    ...
    public static ChunkChangeSet plan(
            ChunkWorkUnit unit,
    ...
    private static int initialCapacity(long candidateCount) {
        return (int) Math.max(1L, Math.min(candidateCount, 4096L));
    }
    ...
    private static final class Accumulator {
        ...
        private Accumulator(int chunkX, int chunkZ, int capacity) {
            ...
            before = new int[capacity];
            after = new int[capacity];
        }
        ...
        private void add(int x, int y, int z, String oldState, String newState) {
            ...
            before[size] = paletteIndex(oldState);
            after[size] = paletteIndex(newState);
            size++;
        }
        ...
        private ChunkChangeSet build() {
            ...
            return new ChunkChangeSet(
                    chunkX,
                    chunkZ,
                    Arrays.asList(orderedPalette),
                    Arrays.copyOf(positions, size),
                    Arrays.copyOf(before, size),
                    Arrays.copyOf(after, size)
            );
        }
    }
}
