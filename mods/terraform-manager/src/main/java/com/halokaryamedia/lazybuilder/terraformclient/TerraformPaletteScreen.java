...
final class TerraformPaletteScreen extends Screen {
    ...
    @Override public void render(DrawContext c,int mouseX,int mouseY,float delta){
        ...
        int yy=Y+43;
        for(TerrainTool tool:TerrainTool.values()){row(c,X+10,yy,W-20,ROW,tool.name(),s.tool()==tool);yy+=ROW+4;}
        c.drawText(textRenderer,"Options",X+12,yy+5,TerraformUi.TEXT_MUTED,false);yy+=19;
        option(c,"Size",String.valueOf((int)s.size()),yy);yy+=24;
        option(c,"Height",String.valueOf((int)s.height()),yy);yy+=24;
        c.drawText(textRenderer,"Variation  "+pretty(s.variation().name()),X+12,yy+5,TerraformUi.TEXT_SECONDARY,false);yy+=24;

        String status=s.busy()?"Applying terrain…":"Ready";
        c.drawText(textRenderer,status,X+12,yy+5,s.busy()?TerraformUi.ACCENT_BRIGHT:TerraformUi.TEXT_MUTED,false);
        action(c,X+118,yy,62,18,"Undo",s.undoAvailable());
        yy+=25;
        c.drawText(textRenderer,"Mouse",X+12,yy+4,TerraformUi.TEXT_MUTED,false);yy+=14;
        c.drawText(textRenderer,"LMB Draw   RMB Flip",X+12,yy+4,TerraformUi.TEXT_SECONDARY,false);yy+=13;
        c.drawText(textRenderer,"Wheel Size   Shift+Wheel Height",X+12,yy+4,TerraformUi.TEXT_MUTED,false);
    ...
    @Override public boolean mouseClicked(double mx,double my,int button){
        ...
        TerraformEditorState s=TerraformManagerClient.state();
        int yy=Y+43;
        for(TerrainTool tool:TerrainTool.values()){
            if(inside(mx,my,X+10,yy,W-20,ROW)){TerraformInteractionController.cancelStroke();s.setTool(tool);return true;}
            yy+=ROW+4;
        }
        yy+=19;
        if(inside(mx,my,X+132,yy,22,18)){s.adjustSize(-1);return true;}
        if(inside(mx,my,X+158,yy,22,18)){s.adjustSize(1);return true;}
        yy+=24;
        if(inside(mx,my,X+132,yy,22,18)){s.adjustHeight(-2);return true;}
        if(inside(mx,my,X+158,yy,22,18)){s.adjustHeight(2);return true;}
        yy+=24;
        if(inside(mx,my,X+10,yy,W-20,20)){TerraformInteractionController.cancelStroke();TerrainVariation[] values=TerrainVariation.values();s.setVariation(values[(s.variation().ordinal()+1)%values.length]);return true;}
        yy+=24;
        if(s.undoAvailable()&&inside(mx,my,X+118,yy,62,18)){TerraformInteractionController.undo();return true;}
    ...
    private void action(DrawContext c,int x,int y,int w,int h,String text,boolean enabled){c.fill(x,y,x+w,y+h,enabled?TerraformUi.ACCENT_FILL:TerraformUi.SURFACE_2);c.fill(x,y,x+2,y+h,enabled?TerraformUi.ACCENT:TerraformUi.BORDER);c.drawCenteredTextWithShadow(textRenderer,text,x+w/2,y+5,enabled?TerraformUi.TEXT_PRIMARY:TerraformUi.TEXT_MUTED);}
