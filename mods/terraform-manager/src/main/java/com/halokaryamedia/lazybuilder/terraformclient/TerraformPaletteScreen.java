...
final class TerraformPaletteScreen extends Screen {
    private static final int X=16,Y=22,W=184,ROW=22;
    TerraformPaletteScreen(){super(Text.literal("LazyBuilder Terraform"));}
    @Override public boolean shouldPause(){return false;}
    @Override public void render(DrawContext c,int mouseX,int mouseY,float delta){
        TerraformEditorState s=TerraformManagerClient.state();panel(c,X,Y,W,206);c.drawText(textRenderer,"TERRAFORM",X+12,Y+10,TerraformUi.TEXT_PRIMARY,false);c.drawText(textRenderer,"Shape",X+12,Y+30,TerraformUi.TEXT_MUTED,false);
        ...
        c.drawText(textRenderer,"Wheel Size   Shift+Wheel Height",X+12,Y+190,TerraformUi.TEXT_MUTED,false);
        super.render(c,mouseX,mouseY,delta);
    }
    @Override public boolean mouseClicked(double mx,double my,int button){if(button!=0)return super.mouseClicked(mx,my,button);TerraformEditorState s=TerraformManagerClient.state();int yy=Y+43;for(TerrainTool tool:TerrainTool.values()){if(inside(mx,my,X+10,yy,W-20,ROW)){s.setTool(tool);return true;}yy+=ROW+4;}
        yy+=19;if(inside(mx,my,X+116,yy,22,18)){s.adjustSize(-1);return true;}if(inside(mx,my,X+142,yy,22,18)){s.adjustSize(1);return true;}yy+=24;if(inside(mx,my,X+116,yy,22,18)){s.adjustHeight(-2);return true;}if(inside(mx,my,X+142,yy,22,18)){s.adjustHeight(2);return true;}yy+=24;if(inside(mx,my,X+10,yy,W-20,20)){TerrainVariation[] values=TerrainVariation.values();s.setVariation(values[(s.variation().ordinal()+1)%values.length]);return true;}return super.mouseClicked(mx,my,button);}
    @Override public void close(){if(client!=null)client.setScreen(null);}
    private void option(DrawContext c,String label,String value,int y){c.drawText(textRenderer,label,X+12,y+5,TerraformUi.TEXT_SECONDARY,false);c.drawText(textRenderer,value,X+86,y+5,TerraformUi.TEXT_PRIMARY,false);small(c,X+116,y,"-");small(c,X+142,y,"+");}
