...
public final class MountainShape implements ShapeField {
    ...
    public double sample(double x, double y, double z) {
        ...
        double body = Math.max(0.0, 1.0 - radius);
        body = Math.pow(body, 1.28);

        double ridgeBias = Math.max(0.0, 1.0 - Math.abs(fz) * 2.3)
                * Math.max(0.0, 1.0 - Math.abs(fx + 0.12) * 1.25);
        double meso = SeededNoise.value2D(x / 18.0, z / 18.0, spec.seed() ^ 0x1F83D9ABL)
                * spec.variation().meso();
        double heightFactor = body * (1.0 + ridgeBias * 0.18 + meso * 0.065);
        double micro = SeededNoise.value2D(x / 8.0, z / 8.0, spec.seed() ^ 0x5BE0CD19L)
                * spec.height() * 0.025 * spec.variation().micro() * body;
        double surfaceHeight = Math.max(spec.height() * 0.02, spec.height() * heightFactor + micro);

        double outsideFootprint = radius - 1.12;
        double outsideBottom = -localY;
        double outsideTop = localY - surfaceHeight;
        return Math.max(outsideFootprint * spec.size(), Math.max(outsideBottom, outsideTop));
    }

    public ShapeBounds bounds() {
        double radius = spec.size() * 1.18 + 2.0;
        return new ShapeBounds(
                spec.origin().x() - radius,
                spec.origin().y(),
                spec.origin().z() - radius,
                spec.origin().x() + radius,
                spec.origin().y() + spec.height() * 1.30 + 3.0,
                spec.origin().z() + radius);
    }
}
