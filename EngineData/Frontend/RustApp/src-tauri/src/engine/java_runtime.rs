...
pub fn ensure_managed_java() -> Result<PathBuf, String> {
    ...
    if target.exists() {
        ...
        match fs::rename(&staging, &target) {
            Ok(()) => { let _ = fs::remove_dir_all(backup); }
            Err(error) => { let _ = fs::rename(&backup, &target); return Err(error.to_string()); }
        }
    } else {
...
fn latest_release() -> Result<JavaRelease, String> {
    let response = ureq::get(ASSETS_URL)
        .set("User-Agent", "LazyBuilder/0.1.0")
        .call()
        .map_err(|e| format!("Adoptium Java lookup failed: {e}"))?;
    let value: Value = response.into_json().map_err(|e| e.to_string())?;
    let release = value.as_array().and_then(|items| items.first())
        .ok_or_else(|| "No Temurin Java 21 release was returned".to_string())?;
    let binary = release.get("binaries").and_then(Value::as_array).and_then(|items| items.first())
        .ok_or_else(|| "Temurin Java binary metadata is missing".to_string())?;
    let package = binary.get("package").ok_or_else(|| "Temurin package metadata is missing".to_string())?;
    let link = package.get("link").and_then(Value::as_str)
        .ok_or_else(|| "Temurin package URL is missing".to_string())?.to_string();
    let checksum = package.get("checksum").and_then(Value::as_str)
        .ok_or_else(|| "Temurin package checksum is missing".to_string())?.to_lowercase();
...
fn extract_zip_stripping_root(archive: &Path, target: &Path) -> Result<(), String> {
    ...
    for index in 0..zip.len() {
        ...
        let enclosed = entry.enclosed_name().ok_or_else(|| "Java archive contains an unsafe path".to_string())?;
...
fn validate_java_21(java: &Path) -> Result<(), String> {
    let output = std::process::Command::new(java).arg("-version").output().map_err(|e| e.to_string())?;
