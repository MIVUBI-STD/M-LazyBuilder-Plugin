...
impl PluginManagerState {
    ...
    pub fn update(&self, plugin_id: &str, jar_path: &str) -> Result<PluginInstallResult, String> {
        ...
        if existing.len() > 1 {
            return Ok(failed_result(
                canonical_id,
                "Duplicate plugin versions must be resolved before updating.",
            ));
        }
        ...
        let target_dir = if current.enabled {
            plugins_directory(&workspace)
        } else {
            disabled_directory(&workspace)
        };
    ...
    pub fn set_enabled(&self, plugin_id: &str, enabled: bool) -> Result<(), String> {
        ...
        let file_name = current
            .path
            .file_name()
            .ok_or_else(|| "Plugin filename is invalid.".to_string())?;
    ...
    pub fn remove(&self, plugin_id: &str) -> Result<(), String> {
        ...
        let backup = rollback_backup_path(&workspace, &canonical_id);
        publish_backup(&current.path, &backup)?;
        fs::remove_file(&current.path).map_err(|error| format!("Could not remove plugin JAR: {error}"))
    }
    ...
    pub fn resolve_duplicates(
        ...
        let Some(keep_index) = existing.iter().position(|item| {
            ...
            return Ok(failed_result(
                canonical_id,
                "Selected JAR is not one of the duplicate candidates.",
            ));
        };
        ...
        for (index, item) in existing.iter().enumerate() {
            ...
            if !files_equal(&item.path, &backup)? {
                ...
                return Err("Duplicate resolution rollback verification failed.".into());
            }
    ...
    for (id, items) in groups {
        ...
        result.push(PluginSummary {
            ...
            category: known_category(&id)
                .or_else(|| known_category(&normalize_id(&metadata.name)))
                .unwrap_or("Other")
                .into(),
            state,
...
fn scan_dir(directory: &Path, enabled: bool) -> Result<Vec<ScannedPlugin>, String> {
    ...
    for entry in fs::read_dir(directory).map_err(|error| error.to_string())? {
        ...
        match read_metadata(&path) {
            Ok(metadata) => result.push(ScannedPlugin {
                path,
                metadata: Some(metadata),
                enabled,
                error: None,
            }),
            Err(error) => result.push(ScannedPlugin {
                path,
                metadata: None,
                enabled,
                error: Some(error),
...
fn read_metadata(path: &Path) -> Result<PluginMetadata, String> {
    ...
    let parsed: PluginYaml =
        serde_yaml::from_str(&text).map_err(|error| format!("Invalid plugin metadata: {error}"))?;
...
fn find_all_by_id(workspace: &Path, canonical_id: &str) -> Result<Vec<ScannedPlugin>, String> {
    ...
    Ok(scanned
        ...
        .filter(|item| {
            item.metadata
                .as_ref()
                .map(|metadata| metadata.id.eq_ignore_ascii_case(canonical_id))
                .unwrap_or(false)
        })
        .collect())
}
...
fn dependency_problem(
    ...
    for dependency in &metadata.dependencies {
        if self_id
            .map(|value| dependency.eq_ignore_ascii_case(value))
            .unwrap_or(false)
        {
            continue;
        }
...
fn known_category(id: &str) -> Option<&'static str> {
    match id {
        ...
        "axiom" | "axiompaper" | "fastasyncworldedit" | "fawe" | "fastasyncvoxelsniper"
        | "ezedits" | "metabrushes" => Some("Build Tools"),
        _ => None,
    }
}
