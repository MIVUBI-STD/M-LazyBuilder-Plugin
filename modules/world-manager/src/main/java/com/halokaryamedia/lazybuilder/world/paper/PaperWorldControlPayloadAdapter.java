...
public final class PaperWorldControlPayloadAdapter implements PluginMessageListener {
    ...
    public void onPluginMessageReceived(String channel, Player player, byte[] message) {
        ...
        if (request instanceof WorldControlWireProtocol.CloneWorld clone) { handleClone(player, clone); return; }
    ...
    private WorldControlWireProtocol.Response handle(Player player, WorldControlWireProtocol.Request request) {
        return switch (request) {
            ...
            case WorldControlWireProtocol.CreateWorld create -> {
                ...
                WorldRecord world = creation.create(create.folderName(), create.displayName(), WorldKind.valueOf(create.kind().strip().toUpperCase(Locale.ROOT)));
                yield new WorldControlWireProtocol.WorldChanged("CREATE", summary(world));
            }
            case WorldControlWireProtocol.LoadWorld load -> { requireManage(player); yield new WorldControlWireProtocol.WorldChanged("LOAD", summary(runtime.load(new WorldId(load.worldId())))); }
            case WorldControlWireProtocol.UnloadWorld unload -> { requireManage(player); yield new WorldControlWireProtocol.WorldChanged("UNLOAD", summary(runtime.unload(new WorldId(unload.worldId())))); }
            case WorldControlWireProtocol.TeleportWorld teleportRequest -> { requireTeleport(player); yield new WorldControlWireProtocol.TeleportOk(summary(teleport.teleportToWorld(player.getUniqueId(), new WorldId(teleportRequest.worldId())))); }
            case WorldControlWireProtocol.ArchiveWorld archive -> { requireManage(player); yield new WorldControlWireProtocol.WorldChanged("ARCHIVE", summary(lifecycle.archive(new WorldId(archive.worldId())))); }
            case WorldControlWireProtocol.RestoreWorld restore -> { requireManage(player); yield new WorldControlWireProtocol.WorldChanged("RESTORE", summary(lifecycle.restore(new WorldId(restore.worldId())))); }
            case WorldControlWireProtocol.GetSettings settings -> { requireManage(player); yield settingsSummary(settingsService.snapshot(new WorldId(settings.worldId()))); }
            case WorldControlWireProtocol.SetAutoLoad setting -> { requireManage(player); settingsService.setAutoLoad(new WorldId(setting.worldId()), setting.enabled()); yield settingsSummary(settingsService.snapshot(new WorldId(setting.worldId()))); }
            case WorldControlWireProtocol.SetDefaultMode setting -> { requireManage(player); settingsService.setDefaultGameMode(new WorldId(setting.worldId()), WorldGameMode.valueOf(setting.gameMode().toUpperCase(Locale.ROOT))); yield settingsSummary(settingsService.snapshot(new WorldId(setting.worldId()))); }
            case WorldControlWireProtocol.SetDifficulty setting -> { requireManage(player); settingsService.setDifficulty(new WorldId(setting.worldId()), WorldDifficulty.valueOf(setting.difficulty().toUpperCase(Locale.ROOT))); yield settingsSummary(settingsService.snapshot(new WorldId(setting.worldId()))); }
            case WorldControlWireProtocol.SetPvp setting -> { requireManage(player); settingsService.setPvp(new WorldId(setting.worldId()), setting.enabled()); yield settingsSummary(settingsService.snapshot(new WorldId(setting.worldId()))); }
            case WorldControlWireProtocol.ResetBuildReady setting -> { requireManage(player); yield settingsSummary(settingsService.resetToBuildReady(new WorldId(setting.worldId()))); }
            case WorldControlWireProtocol.SetSpawnHere setting -> { requireManage(player); settingsService.setSpawnToPlayer(player.getUniqueId(), new WorldId(setting.worldId())); yield settingsSummary(settingsService.snapshot(new WorldId(setting.worldId()))); }
    ...
    private boolean beginHeavy(Player player) {
        try { requireManage(player); } catch (RuntimeException exception) { send(player, WorldControlWireProtocol.error(exception.getMessage())); return false; }
        if (!heavyInFlight.add(player.getUniqueId())) { send(player, WorldControlWireProtocol.error("Another World Manager file operation is still processing")); return false; }
        return true;
    }
    ...
    private void handleDelete(Player player, WorldControlWireProtocol.DeleteWorld request) {
        ...
        try { task = deleteService.prepare(new WorldId(request.worldId()), request.typedFolderName()); deletedSummary = summary(task.world()); }
        catch (RuntimeException exception) { heavyInFlight.remove(owner); send(player, WorldControlWireProtocol.error(exception.getMessage())); return; }
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
    ...
    private void handleExport(Player player, WorldControlWireProtocol.ExportWorld request) {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            plugin.getServer().getScheduler().runTask(plugin, () -> {
                if (finalCaptureFailure != null) { completeHeavy(owner, () -> finishExportFailure(task, finalCaptureFailure)); return; }
                try { exportService.resumeSourceAfterSnapshot(task); }
                catch (Throwable resumeFailure) { completeHeavy(owner, () -> finishExportFailure(task, resumeFailure)); return; }
                plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
                    ...
                    plugin.getServer().getScheduler().runTask(plugin, () -> completeHeavy(owner, () -> {
                        ...
                        if (outcome != null) return WorldControlWireProtocol.error(outcome.getMessage());
                        return encode(new WorldControlWireProtocol.ExportReady(request.worldId(), finalResult.artifact().getFileName().toString(), finalResult.targetFormat()));
                    }));
                });
            });
        });
    ...
    private static Throwable combine(Throwable first, Throwable next) { if (first == null) return next; first.addSuppressed(next); return first; }

    private WorldControlWireProtocol.WorldSummary summary(WorldRecord world) {
        return new WorldControlWireProtocol.WorldSummary(world.id().value(), world.folderName(), world.displayName(), world.kind().name(), world.lifecycle().name(), runtime.state(world.id()).name(), world.autoLoad(), world.defaultGameMode());
    }

    private static WorldControlWireProtocol.SettingsSnapshot settingsSummary(WorldSettingsSnapshot snapshot) {
        var runtime = snapshot.runtime(); var spawn = runtime.spawn();
        return new WorldControlWireProtocol.SettingsSnapshot(snapshot.world().id().value(), snapshot.world().autoLoad(), snapshot.defaultGameMode().name(), runtime.difficulty().name(), runtime.pvpEnabled(), runtime.weather().name(), runtime.timeOfDayTicks(), spawn.x(), spawn.y(), spawn.z());
    }

    private void requireAnyWorldPermission(Player player) { if (!player.hasPermission(MANAGE_PERMISSION) && !player.hasPermission(TELEPORT_PERMISSION)) throw new IllegalStateException("Missing LazyBuilder world permission"); }
    private static void requirePermission(Player player, String permission) { if (!player.hasPermission(permission)) throw new IllegalStateException("Missing permission: " + permission); }
    ...
    private static byte[] encode(WorldControlWireProtocol.Response response) { try { return WorldControlWireProtocol.encodeResponse(response); } catch (IOException impossible) { throw new IllegalStateException(impossible); } }

    private void send(Player player, byte[] payload) {
        if (payload.length > WorldControlWireProtocol.MAX_MESSAGE_BYTES) { plugin.getLogger().warning("Refusing oversized world-control response for " + player.getUniqueId()); return; }
